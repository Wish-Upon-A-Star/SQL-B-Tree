...
공용 API 헤더는 `cmd_processor/cmd_processor.h`로 둔다.
...
이번 단계에서 추가할 파일은 `cmd_processor/` 폴더 아래에 둔다.
...
cmd_processor/mock_cmd_processor.h / cmd_processor/mock_cmd_processor.c
...
cmd_processor/cmd_processor_test.c
  - CmdProcessor public 계약과 mock 구현체 동작 검증
```
