...
- 빠른 mutation path를 유지한다.
- 일반 SQL은 `parse_statement()` 후 executor로 넘긴다.
...
주의: v1에서는 executor 전체를 구조화된 결과 모델로 바꾸지 않는다. 기존 출력 중심 실행을 보존하되, stdout 대신 output buffer로 보낼 수 있는 최소 변경을 한다.

### 4.2 Executor 출력 sink

현재 `executor.c`는 `printf()`와 `INFO_PRINTF()`에 직접 의존한다. REPL 응답 body를 만들려면 출력 목적지를 바꿀 수 있어야 한다.

v1 기본안:

```c
typedef void (*ExecutorWriteFn)(void *ctx, const char *text);

void set_executor_output(ExecutorWriteFn write_fn, void *ctx);
void reset_executor_output(void);
```

출력 규칙:

- 기본 상태에서는 기존처럼 stdout에 쓴다.
- `REPLCmdProcessor.submit()` 동안만 output buffer writer를 설정한다.
- SQL eval 구간은 mutex로 보호한다. executor 출력 sink가 전역 상태이기 때문이다.
- `INFO_PRINTF()`도 같은 sink를 사용한다.

stdout redirection은 사용하지 않는다. 프로세스 전역 fd를 바꾸면 테스트가 불안정하고, 다른 출력과 섞일 수 있다.
...
| 경우 | status | body |
| --- | --- | --- |
| 정상 실행 | `CMD_STATUS_OK` | executor 출력 text |
...
1. `cmd_processor/sql_repl_engine.h/.c`를 추가하고 `main.c`의 SQL 한 문장 실행 로직을 이동 가능한 형태로 정리한다.
2. executor 출력 sink를 추가해 stdout 대신 buffer에 출력할 수 있게 한다.
...
SQL 실행 결과를 얻기 위해 executor 출력 sink를 추가한다.
