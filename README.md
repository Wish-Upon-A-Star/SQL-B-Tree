...
README의 기술적 신뢰도를 높일 수 있도록, 핵심 기술 명칭을 명확히 하고 보고서 형식의 명사형 어미로 정리했습니다.
...
* 캐시 범위를 초과하는 데이터는 'Uncached tail' 영역으로 분류하여 필요 시에만 파일 경로를 통해 접근함.
* 메모리 폭주(OOM)를 방지하는 동시에 빈번하게 접근하는 상위 데이터의 응답 속도를 확보함.

### 2. 페이지 캐시 및 레이지 로우(Lazy Row) 로딩
* 데이터 전체 문자열을 항상 유지하지 않고, 실제 요청이 발생하는 시점에 로드하는 지연 로딩 방식 채택함.
* 페이지 캐시를 통해 파일의 특정 페이지만 선별적으로 읽어 재사용 효율을 극대화함.
* 레코드 오프셋(Offset) 정보를 관리하여 동일 데이터 재조회 시 파일 전체 스캔 과정을 생략함.

### 3. 미캐싱 영역 대상 PK 오프셋 관리
* 메모리 외부에 존재하는 데이터(Uncached tail)에 대해서도 PK(Primary Key) 기반의 위치 정보를 별도로 기억함.
* 데이터 본문이 메모리에 없더라도 `WHERE id = ...`와 같은 단일 조회 시 즉각적인 파일 탐색이 가능하도록 설계함.
...
### 5. Append-only 델타 로그 및 슬롯(Slot) 재사용
...
- 4명의 팀을 다시 2명의 팀 단위로 나누어 진행해 본 시도.
- 발표 문서에 Mermaid 다이어그램과 의사코드를 함께 넣어, 코드 전체를 읽지 않아도 구조를 이해할 수 있게 하려 한 노력.
